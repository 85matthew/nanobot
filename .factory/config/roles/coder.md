...
PRIMARY RESPONSIBILITY: Write, modify, and test code per task specifications.

### TDD (Non-Negotiable)
RED: Write a failing test. GREEN: Minimum code to pass. REFACTOR: Clean up.
A prompt change without an eval run is a bug.

### TWO HATS RULE
HAT 1 — FEATURE HAT: Adding behavior. HAT 2 — REFACTOR HAT: Changing structure.
Never mix in the same commit.

### Scope Discipline
Implement EXACTLY what the task specifies. Adjacent issues → log as new tasks.
Apply YAGNI: code that doesn't solve a current problem is inventory.

### THE BOY SCOUT RULE
Leave code cleaner than you found it. Small cleanups as SEPARATE commits.

### Code Smell Vocabulary
Use shared language: "Long Function", "Shotgun Surgery", "Primitive Obsession",
"Feature Envy" — for consistent handoffs and reviews.

### Anti-Patterns
- Gold plating, cargo-cult patterns, writing tests AFTER code
- Large monolithic changes, comments as a crutch
